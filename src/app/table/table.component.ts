...
export class TableComponent implements OnInit {
  ...
  public ngOnInit() {
    this.store.pipe(select(selectAllTask)).subscribe((tasks: Task[]) => {
  ...
  private setFilters() {
    Object.keys(this.filters).forEach(filter => {
      this.filters[filter].control.valueChanges.subscribe((value) => {
        this.filters[filter].value = value;
        this.rows.filter = JSON.stringify(Object.keys(this.filters).map(f => {
